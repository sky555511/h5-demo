...
- 卡片式订单列表，状态仅两种：`待回收`（可编辑、可生成二维码）、`已回收`（仅二维码）
- 生成订单二维码（可下载 PNG 图片）
...
提交成功页展示订单二维码，支持「保存二维码图片」，供应商可截图保存。
...
- **订单字段**：订单结构对齐网页版 `outInstrumentOrder`（`orderStatus`：0 未确认可编辑 / 1 已确认；`recycleTime` 非空表示已回收）

## 已知取舍（demo 范畴）

- 二次修改在保存后订单仍为「待回收」状态（与网页版一致：已回收订单前端不可编辑）
- 订单管理未提供删除按钮（网页版删除能力未纳入本轮需求，需要可加）
